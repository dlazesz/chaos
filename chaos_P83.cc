...
// the projected image on the plane. The estimation itself:
 ...
 estimation.y+=interval(-2,2)*T*T;   // Generously overestimated
...
// check the stricter rules for both!
  ...
  (lOFlinRx(p1)== lOFlinRx(p2)  ||  ((!NOTinRx(p1) ||  !NOTinRx(p2))   &&
       inRy(p1)&&     inRy(p2)))&&
  (lOFrinLx(p1)== lOFrinLx(p2)  ||  ((!NOTinLx(p1) ||  !NOTinLx(p2))   &&
       inLy(p1)&&     inLy(p2)))&&
  (rOFlinLx(p1)== rOFlinLx(p2)  ||  ((!NOTinLx(p1) ||  !NOTinLx(p2))   &&
       inLy(p1)&&     inLy(p2)))&&
  (rOFrinLx(p1)== rOFrinLx(p2)  ||  ((!NOTinLx(p1) ||  !NOTinLx(p2))   &&
       inLy(p1)&&     inLy(p2)))&&
  (lOFlinLx(p1)== lOFlinLx(p2)  ||  ((!NOTinLx(p1) ||  !NOTinLx(p2))   &&
       inLy(p1)&&     inLy(p2)))&&
// These are the wertical checks
  ...
  ((NOTinLx(p1)&&  NOTinLx(p2)) ||    lOFtinLy(p1) ==  lOFtinLy(p2))   &&
  ((NOTinLx(p1)&&  NOTinLx(p2)) ||    uOFbinLy(p1) ==  uOFbinLy(p2))   &&
  ((NOTinLx(p1)&&  NOTinLx(p2)) ||    uOFtinLy(p1) ==  uOFtinLy(p2))   &&
  ((NOTinLx(p1)&&  NOTinLx(p2)) ||    lOFbinLy(p1) ==  lOFbinLy(p2))   &&
